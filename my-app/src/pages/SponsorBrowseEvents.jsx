...
export default function SponsorBrowseEvents() {
    ...
    useEffect(() => {
        ...
        if (parsed.selectedStates?.length > 0) setSelectedState(parsed.selectedStates[0]);
    }, [navigate]);

    // ✅ Poll every second — new college events appear instantly
    useEffect(() => {
    ...
    const filteredEvents = allEvents.filter(event => {
        ...
        const matchesBudgetMin = !budgetMin || (event.maxSponsorship && event.maxSponsorship >= Number(budgetMin));
        const matchesBudgetMax = !budgetMax || (event.minSponsorship && event.minSponsorship <= Number(budgetMax));

        return matchesSearch && matchesCategory && matchesState && matchesCity && matchesBudgetMin && matchesBudgetMax;
    });

    const hasActiveFilters = searchQuery || category !== 'all' || selectedState !== 'all' || selectedCity || budgetMin || budgetMax;

    const clearAllFilters = () => {
        setSearchQuery(''); setCategory('all'); setSelectedState('all');
        setSelectedCity(''); setBudgetMin(''); setBudgetMax('');
    };
                        ...
                        <Card className="p-4 mb-6">
                            <div className="flex flex-col md:flex-row gap-4">
                                <div className="relative flex-1">
                                    ...
                                    <Input placeholder="Search events, colleges, locations..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10" />
                                    {searchQuery && (
                                ...
                                <Button variant="outline" onClick={() => setShowFilters(!showFilters)} className={showFilters ? 'border-[#1E3A8A] text-[#1E3A8A]' : ''}>
                                ...
                                <div className="mt-4 pt-4 border-t">
                                    <div className="mb-5">
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {categories.map(cat => (
                                                <Badge key={cat} variant={category === cat.toLowerCase() ? 'default' : 'outline'}
                                                    className={`cursor-pointer transition-colors ${category === cat.toLowerCase() ? 'bg-[#1E3A8A] text-white border-[#1E3A8A]' : 'hover:border-[#1E3A8A] hover:text-[#1E3A8A]'}`}
                                        ...
                                        <div className="grid md:grid-cols-2 gap-3">
                                            ...
                                            <div>
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <Input placeholder="Search city..." value={selectedCity} onChange={e => setSelectedCity(e.target.value)} className="pl-9" />
                                                    {selectedCity && (
                                                        <button onClick={() => setSelectedCity('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400">
                                                            <X className="w-3 h-3" />
                                                        </button>
                                                    )}
                                                </div>
                                    ...
                                    <div className="mb-4">
                                        <p className="text-xs text-slate-500 font-medium uppercase tracking-wide mb-1">Sponsorship Budget Range (₹)</p>
                                        <div className="grid grid-cols-2 gap-3">
                                            <div>
                                                <p className="text-xs text-slate-400 mb-1">Min Budget (₹)</p>
                                                <Input type="number" placeholder="e.g. 5000" value={budgetMin} onChange={e => setBudgetMin(e.target.value)} min="0" />
                                            </div>
                                            <div>
                                                <p className="text-xs text-slate-400 mb-1">Max Budget (₹)</p>
                                                <Input type="number" placeholder="e.g. 50000" value={budgetMax} onChange={e => setBudgetMax(e.target.value)} min="0" />
                                            </div>
                                        </div>
                                        <div className="flex flex-wrap gap-2 mt-2">
                                            {[
                                                ...
                                                <button key={preset.label} type="button" onClick={() => { setBudgetMin(preset.min); setBudgetMax(preset.max); }}
                                                    className={`px-3 py-1 rounded-full text-xs border transition-colors ${budgetMin === preset.min && budgetMax === preset.max ? 'bg-[#1E3A8A] text-white border-[#1E3A8A]' : 'bg-white text-slate-600 border-slate-300 hover:border-[#1E3A8A] hover:text-[#1E3A8A]'}`}>
                                                    {preset.label}
                                                </button>
                                            ))}
                                        </div>
                                    ...
                                    {hasActiveFilters && (
                                        <div className="flex justify-end pt-3 border-t">
                                            <Button variant="ghost" onClick={clearAllFilters} className="text-red-500 hover:text-red-600 hover:bg-red-50 text-sm">
                            ...
                            <div className="flex flex-wrap gap-2 mb-4">
                                {searchQuery && (
                                    <span className="px-3 py-1 bg-[#1E3A8A]/10 text-[#1E3A8A] rounded-full text-xs flex items-center gap-1">
                                        Search: "{searchQuery}" <button onClick={() => setSearchQuery('')}><X className="w-3 h-3" /></button>
                                    </span>
                                )}
                                {category !== 'all' && (
                                    <span className="px-3 py-1 bg-[#1E3A8A]/10 text-[#1E3A8A] rounded-full text-xs flex items-center gap-1">
                                        Category: {category} <button onClick={() => setCategory('all')}><X className="w-3 h-3" /></button>
                                    </span>
                                )}
                                {selectedState !== 'all' && (
                                    <span className="px-3 py-1 bg-[#1E3A8A]/10 text-[#1E3A8A] rounded-full text-xs flex items-center gap-1">
                                        <MapPin className="w-3 h-3" /> {selectedState} <button onClick={() => setSelectedState('all')}><X className="w-3 h-3" /></button>
                                    </span>
                                )}
                                {selectedCity && (
                                    <span className="px-3 py-1 bg-[#1E3A8A]/10 text-[#1E3A8A] rounded-full text-xs flex items-center gap-1">
                                        <MapPin className="w-3 h-3" /> {selectedCity} <button onClick={() => setSelectedCity('')}><X className="w-3 h-3" /></button>
                                    </span>
                                )}
                                {(budgetMin || budgetMax) && (
                                    <span className="px-3 py-1 bg-[#1E3A8A]/10 text-[#1E3A8A] rounded-full text-xs flex items-center gap-1">
                                        ₹{budgetMin || '0'} – ₹{budgetMax || '∞'} <button onClick={() => { setBudgetMin(''); setBudgetMax(''); }}><X className="w-3 h-3" /></button>
                                    </span>
                                )}
                                <button onClick={clearAllFilters} className="px-3 py-1 text-red-500 text-xs underline">Clear all</button>
                            </div>
                        )}

                        <p className="text-slate-600 mb-6">
                            Showing <span className="font-semibold text-[#1F2937]">{filteredEvents.length}</span>
                            {hasActiveFilters && ` of ${allEvents.length}`} events
                        </p>

                        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {filteredEvents.map(event => (
                                <EventCard key={event.id} event={event} showApplyButton onApply={handleApply} />
                            ))}
                        </div>
